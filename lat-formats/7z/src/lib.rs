...
use sevenz_rust::{SevenZWriter, SevenZReader, SevenZArchiveEntry};
use std::io::Cursor;
...
impl Compressor for SevenZCompressor {
    fn compress(&self, entries: &[ArchiveEntry], _password: Option<&str>) -> Result<Vec<u8>, String> {
        ...
        for entry in entries {
            ...
            writer.push_archive_entry(sz_entry, Some(Cursor::new(&entry.data)))
    ...
    fn decompress(&self, archive_data: &[u8], _password: Option<&str>) -> Result<Vec<ArchiveEntry>, String> {
        ...
        reader.for_each_entries(|file, reader| {
            ...
            std::io::Read::read_exact(reader, &mut buf).map_err(|e| std::io::Error::new(std::io::ErrorKind::Other, e.to_string()))?;
